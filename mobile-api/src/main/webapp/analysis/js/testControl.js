...
function start() {
	...
	descriptionStr = $("#sampleId").val() + ", " +  $("#windSpeed").val() + ", " + $("#testDescription").val();
	...
	$.ajax(
			{
				...
				success : function(data) {
					if (data.msId != null) {
						link = "<a target='_blank' href='/analysis/measurement?pass=2gh7yJfJ6H&session_id="
								+ data.msId
								+ "'>Result "
								+ data.msId
								+ "</a>";
...
$(document).ready(function() {
	...
	watch = new Stopwatch(stopwatchElement, {
		delay : 500,
		call : getFreqUpdate
	});
});