...
function drawVisualization() {
	...
	var Frequency = new google.visualization.ChartWrapper({
		...
		"containerId" : "Frequency",
		"options" : {
			"pointSize" : 1,
			"vAxis" : {
				"title" : "Rotation Freq (Hz)/(m/s)"
			},
			"lineWidth" : 1
		}
	});
	...
	function jsFreqAnalysis() {
		...
		for (var i = 0; i < freqCols.length; i++) {
			...
			mins[i] = min;
			stds[i] = Math
					.sqrt(1 / rows[i].length * SS - Math.pow(means[i], 2));
		}
		...
		for (var i = 0; i < freqCols.length; i++) {
			...
			analysis += "<tr><td>" + name + "</td><td>" + means[i].toPrecision(4)
					+ "</td><td>" + maxs[i].toPrecision(4) + "</td><td>" + mins[i].toPrecision(4)
					+ "</td><td>" + stds[i].toPrecision(4) + "</td></tr>";
		}
